...
public class GitLink extends Plugin {
    ...
    public void populateMenu(JMenu menu, int flags) {
        ...
        if (flags == (Plugin.MENU_TOOLS | Plugin.MENU_BOTTOM)) {
            JMenuItem item = new JMenuItem("My GIT SSH Key");
            item.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    showPublicKey();
                }
    ...
    public void showPublicKey() {
        File ssh = Base.getDataFile(".ssh");
        File pub = new File(ssh, "id_rsa.pub");

        String p = Base.getFileAsString(pub);

        
        StringSelection stringSelection = new StringSelection (p);
        Clipboard clpbrd = Toolkit.getDefaultToolkit ().getSystemClipboard ();
        clpbrd.setContents (stringSelection, null);
        editor.message("Your key has been copied to the clipboard.");
    }
    ...
    public void loadOrGenerateKeys() {
        ...
        final File pub = new File(ssh, "id_rsa.pub");
        final File key = new File(ssh, "id_rsa");
        try {
    
            if (!pub.exists()) {
                editor.warning("Generating RSA key pair...");
                keyPair = KeyPair.genKeyPair(new JSch(),KeyPair.RSA, 2048);
                keyPair.writePublicKey(pub.getAbsolutePath(), "UECIDE");
